...
class MainActivity : AppCompatActivity() {
    ...
    // we can't construct the diceImage object until the MainActivity is available in memory,
    // and that doesn't happen until the layout is inflated by setContentView() below
    // declaring this as a lateinit is a promise to the kotlin compiler that we will nto attempt
    // any operations on the var until it is initialized
    // this allows us to  make the variable non-nullable and therefore use the variable
    // without doing a null check every single time
